...
public class GetUserMediaImpl {
    ...
    /**
     * Android 14+ hook: set this to a Runnable that calls startForeground(MEDIA_PROJECTION) on
     * the running foreground service. It is invoked synchronously in onReceiveResult, immediately
     * before getMediaProjection() is called, so the Binder update to AMS is guaranteed to complete
     * before the system checks for a MEDIA_PROJECTION foreground service.
     */
    public static Runnable onMediaProjectionGranted = null;
            ...
            screenRequestPermissions(
                    new ResultReceiver(new Handler(Looper.getMainLooper())) {
                        ...
                        protected void onReceiveResult(int requestCode, Bundle resultData) {
                            ...
                            // Android 14+: call startForeground(MEDIA_PROJECTION) synchronously
                            // here, before getMediaProjection() is invoked inside getDisplayMedia.
                            // startForeground() is a synchronous Binder call to AMS, so by the
                            // time it returns the service type is updated and getMediaProjection()
                            // will find the required MEDIA_PROJECTION foreground service.
                            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.UPSIDE_DOWN_CAKE
                                    && onMediaProjectionGranted != null) {
                                onMediaProjectionGranted.run();
                            }
    ...
    private void getDisplayMedia(final Result result, final MediaStream mediaStream, final Intent mediaProjectionData) {
        ...
        // Cap the capture resolution to avoid NO_MEMORY codec failures on high-DPI screens.
        // Encoding at native resolution (e.g. 1920x1200) exhausts the codec's graphics buffer
        // pool. Scale down so the longer edge is at most 1280px, keeping the aspect ratio and
        // rounding each dimension to the nearest even number required by the H.264 encoder.
        int capWidth = size.x;
        int capHeight = size.y;
        final int MAX_LONG_EDGE = 1280;
        int longEdge = Math.max(capWidth, capHeight);
        if (longEdge > MAX_LONG_EDGE) {
            float scale = (float) MAX_LONG_EDGE / longEdge;
            capWidth  = ((int)(capWidth  * scale) + 1) & ~1;
            capHeight = ((int)(capHeight * scale) + 1) & ~1;
        }

        VideoCapturerInfoEx info = new VideoCapturerInfoEx();
        info.width = capWidth;
        info.height = capHeight;
        info.fps = DEFAULT_FPS;
        info.isScreenCapture = true;
        info.capturer = videoCapturer;

        videoCapturer.startCapture(capWidth, capHeight, info.fps);
        Log.d(TAG, "OrientationAwareScreenCapturer.startCapture: " + capWidth + "x" + capHeight + "@" + info.fps);
